...
rem Запуск консоли от TrustedInstaller
	...
	Helper /CenterCMD
	Helper /Title "DK | v. %Version% | %DateProgram% | build - %NumberWin% | By Vlado"
...
rem Начало меню
	...
	%ch% {08} ---------------------------------------------------------------------------{\n#}
	%ch% {09} 1. {04}Удалить Защитник{\n#}
	%ch% {09} 2. {08}Проверить состояние папок и файлов{\n#}
	%ch% {09} 3. {0e}Восстановление / Дополнительно{\n#}{\n#}
...
rem # Fix list + Found
	...
	reg query "HKLM\System\CurrentControlSet\Services\WdFilter" >nul 2>&1 && (
		...
		echo.
		%ch% {04}  Часть служб/драйверов не удалена. Требуется перезапуск ПК для подчистки^^!{\n#}
...
:ManageDefender
	...
	%ch% {03} 1. {green}Восстановить защитник из копии{\n#}{\n#}
	%ch% {9f}Безопасность Windows{\n#}
	%ch% {03} 2. %MenuSec% {08}(приложение в пуске){\n#}
...
:RestoreDefender
	sc query WinDefend >nul 2>&1 && (
		...
		%ch% {08} Сначала выполните удаление и перезапустите ПК{\n#}
		call :Pause && goto ManageDefender
	)
...
:CheckSAC
	...
	reg query "HKLM\System\CurrentControlSet\Control\CI\Policy" /v VerifiedAndReputablePolicyState | find /i "0x0" >nul 2>&1 && (
		%ch%  {0c} Требуется перезапустить ПК{\n#}
		call :Pause && goto Start
	)
...
:CheckNeedFiles
	for %%f in (%NeedFiles%) do if not exist %%f (
		...
		pause
		start https://disk.yandex.ru/d/G_IcVLrpDm6bLA
		exit
	)
	exit /b
